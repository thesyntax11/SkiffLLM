...
Java_com_skifflm_app_SkiffNative_create(
        JNIEnv* env,
        jobject,
        jint context_size,
        jint threads,
    ...
    state->config = skifflm::default_config();
...
Java_com_skifflm_app_SkiffNative_destroy(JNIEnv*, jobject, jlong handle) {
    delete state_from(handle);
}

extern "C" JNIEXPORT jboolean JNICALL
Java_com_skifflm_app_SkiffNative_load(
        JNIEnv* env,
        jobject,
        jlong handle,
        jstring model_path) {
    ...
    auto engine = std::make_unique<skifflm::LlmEngine>(state->config);
    std::string error;
...
Java_com_skifflm_app_SkiffNative_loadError(JNIEnv* env, jobject, jlong handle) {
    NativeState* state = state_from(handle);
    if (state == nullptr) {
...
extern "C" JNIEXPORT void JNICALL
Java_com_skifflm_app_SkiffNative_stop(JNIEnv*, jobject, jlong handle) {
    NativeState* state = state_from(handle);
    if (state != nullptr) {
...
extern "C" JNIEXPORT jboolean JNICALL
Java_com_skifflm_app_SkiffNative_generate(
        JNIEnv* env,
        jobject,
        jlong handle,
        jobjectArray roles,
