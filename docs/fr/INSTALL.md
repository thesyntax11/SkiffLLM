...
`skiffllm-<version>-<os>-<arch>.tar.gz` (ou `.zip` sous Windows) ainsi que
`checksums.txt`.
...
Les fichiers sont enregistrés par défaut dans `~/.local/share/skiffllm/models`.
...
```bash
bash scripts/demo-capture.sh ./build/release/skiffllm /chemin/model.gguf
```
