...
| `ollama run <model>` | `skiffllm --model <file>` (or `skiffllm run "<prompt>"`) |
| `ollama serve` | `skiffllm --serve --host 127.0.0.1 --port 8080` |
| `ollama list` | `skiffllm model list` / `skiffllm --list-models` |
| `ollama ps` | no resident daemon — there is no persistent process |
| `ollama show` | `skiffllm --model <file> --model-info` |
| `ollama rm` | `skiffllm model remove <id> --force` |
...
- **You bring the model.** `skiffllm` does not download at runtime. Use
...
2. Record its hash: `python3 scripts/model_fetch.py --verify` or
   `skiffllm model verify <id>`.
3. Run once: `skiffllm --model <file> --model-info`.
4. Move your pipeline: `git diff | skiffllm "review"` instead of
   `curl localhost:11434/api/chat`.
5. For a service, start the local server only where you need it:
   `skiffllm --serve --host 127.0.0.1 --port 8080 --api-key "$KEY"`.
6. Confirm what you adopted with `skiffllm --doctor` and `--backend-info`.
