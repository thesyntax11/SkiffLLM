...
| `ollama run <model>` | `llm --model <file>` (or `llm run "<prompt>"`) |
| `ollama serve` | `llm --serve --host 127.0.0.1 --port 8080` |
| `ollama list` | `llm model list` / `llm --list-models` |
| `ollama ps` | no resident daemon — there is no persistent process |
| `ollama show` | `llm --model <file> --model-info` |
| `ollama rm` | `llm model remove <id> --force` |
...
- **You bring the model.** `llm` does not download at runtime. Use
...
2. Record its hash: `python3 scripts/model_fetch.py --verify` or
   `llm model verify <id>`.
3. Run once: `llm --model <file> --model-info`.
4. Move your pipeline: `git diff | llm "review"` instead of
   `curl localhost:11434/api/chat`.
5. For a service, start the local server only where you need it:
   `llm --serve --host 127.0.0.1 --port 8080 --api-key "$KEY"`.
6. Confirm what you adopted with `llm --doctor` and `--backend-info`.
