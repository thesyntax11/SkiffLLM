...
./build/skifflm --backend-info                     # verbundene Backends anzeigen
```
...
Das Hilfsskript speichert die Datei in `~/.local/share/skifflm/models`. Die
...
Die Standardkonfigurationsdatei liegt unter `~/.config/skifflm/config`. Ein
vollständiges Beispiel befindet sich in `configs/skifflm.example.conf`.
