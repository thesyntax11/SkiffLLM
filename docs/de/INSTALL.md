...
`skifflm-<version>-<os>-<arch>.tar.gz` (unter Windows `.zip`) sowie
`checksums.txt`.
...
Dateien landen standardmäßig in `~/.local/share/skifflm/models`. Sie können mit
...
```bash
bash scripts/demo-capture.sh ./build/release/skifflm /pfad/model.gguf
```
