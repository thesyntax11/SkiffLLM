...
./build/skiffllm --backend-info                     # bağlı arka uçları görüntüle
```
...
Yardımcı, dosyayı `~/.local/share/skiffllm/models` dizinine kaydeder. Çıkarım
...
Varsayılan yapılandırma dosyası `~/.config/skiffllm/config` yolundadır. Tam bir
örnek `configs/skiffllm.example.conf` dosyasındadır. Komut satırı seçenekleri
