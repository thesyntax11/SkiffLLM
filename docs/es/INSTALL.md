...
`skifflm-<version>-<os>-<arch>.tar.gz` (o `.zip` en Windows) además de
`checksums.txt`.
...
Los archivos se guardan en `~/.local/share/skifflm/models` por defecto. También
...
```bash
bash scripts/demo-capture.sh ./build/release/skifflm /ruta/model.gguf
```
