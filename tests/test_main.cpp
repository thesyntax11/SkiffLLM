...
void test_config_file() {
    const auto dir = std::filesystem::temp_directory_path() / "skiffllm-tests";
    ...
    std::string error;
    const bool ok = skiffllm::parse_config_file(path, cfg, error);
...
void test_parse_args() {
    ...
    std::string error;
    const bool ok = skiffllm::parse_args(static_cast<int>(args.size()), args.data(), cfg, error);
...
void test_profiles() {
    ...
    check(skiffllm::apply_profile(cfg, "fast", error), "fast profile should apply");
    ...
    check(skiffllm::apply_profile(cfg, "code", error), "code profile should apply");
    check(approx(cfg.temperature, 0.20f), "code temperature");
    check(cfg.n_predict == 1024, "code n_predict");

    check(!skiffllm::apply_profile(cfg, "missing", error), "unknown profile should fail");
...
void test_parse_args_new_features() {
    skiffllm::Config cfg = skiffllm::default_config();
    std::vector<std::string> storage = {
        "skiffllm",    "--session",     "writing",   "--profile=creative",
...
void test_session_roundtrip() {
    const auto dir = std::filesystem::temp_directory_path() / "skiffllm-session";
    std::filesystem::create_directories(dir);
    ...
    skiffllm::Config cfg2 = skiffllm::default_config();
    cfg2.history_path = path;

    skiffllm::Session session(cfg2);
    std::string error;
...
void test_formatting() {
    check(skiffllm::trim("  hello \t\n") == "hello", "trim should remove whitespace");
    check(skiffllm::lower("AbC") == "abc", "lower should lowercase");
    check(skiffllm::to_human_bytes(1024u) == "1.00 KiB", "bytes should format");
    check(skiffllm::to_human_count(1500u) == "1.50 K", "count should format");
}

void test_server_bearer_auth() {
    check(skiffllm::bearer_token_matches("Bearer local-token", "local-token"),
    ...
    check(!skiffllm::bearer_token_matches("Bearer wrong", "local-token"),
          "wrong bearer token should not match");
    check(!skiffllm::bearer_token_matches("Basic local-token", "local-token"),
          "non-bearer scheme should not match");
    check(!skiffllm::bearer_token_matches("Bearer", "local-token"),
          "missing bearer token should not match");
    check(skiffllm::bearer_token_matches("Bearer anything", ""),
          "empty configured key leaves the server public");
    check(!skiffllm::bearer_token_matches("", "local-token"),
...
void test_advanced_flags() {
    skiffllm::Config cfg = skiffllm::default_config();
    std::vector<std::string> storage = {
        "skiffllm", "--flash-attn", "--numa",  "--no-kv-offload", "--n-keep",    "3",
        "--doctor", "--model-info", "--smoke", "--tokenize",      "hello world",
    };
    auto args = args_from(storage);
    std::string error;
    bool ok = skiffllm::parse_args(static_cast<int>(args.size()), args.data(), cfg, error);
...
void test_popularity_flags() {
    skiffllm::Config cfg = skiffllm::default_config();
    std::vector<std::string> storage = {
        "skiffllm", "--attach",        "notes.txt", "--file=guide.md", "--export",
...
void test_tools_features() {
...

    {
        ...
        auto args = args_from(storage);
        std::string error;
        check(skiffllm::parse_args(static_cast<int>(args.size()), args.data(), cfg, error),
...

    {
        ...
        check(skiffllm::parse_args(static_cast<int>(args.size()), args.data(), memory_cfg,
                                   parse_error),
              "remember flag should parse");
        check(memory_cfg.remember_text == "be concise", "remember text should be stored");

        memory_cfg = skiffllm::default_config();
        storage = {"skiffllm", "--forget", "concise"};
        ...
        memory_cfg = skiffllm::default_config();
        storage = {"skiffllm", "--summarize", "README.md"};
        args = args_from(storage);
        check(skiffllm::parse_args(static_cast<int>(args.size()), args.data(), memory_cfg,
                                   parse_error),
...

    {
        ...
        custom.history_path =
            std::filesystem::temp_directory_path() / "skiffllm-sessions" / "default.skif";

        const auto path = skiffllm::session_file_for(custom, "writing");
...

    {
        ...
        const auto old_file = skiffllm::session_file_for(session_cfg, "writing");
        const auto new_file = skiffllm::session_file_for(session_cfg, "reading");
        {
        ...
        const int status = skiffllm::handle_session_command(session_cfg, args, err);
...

    {
        ...
        auto args = args_from(storage);
        std::string parse_error;
        check(skiffllm::parse_args(static_cast<int>(args.size()), args.data(), code_cfg,
                                   parse_error),
    ...
    check(skiffllm::model_catalog().size() >= 5, "model catalog should be non-empty");
    check(skiffllm::find_catalog_model("qwen2.5-0.5b") != nullptr,
          "catalog should find a known id");
}

void test_config_and_stats_features() {
    const auto dir = std::filesystem::temp_directory_path() / "skiffllm-config-stats-test";
    std::filesystem::remove_all(dir);
    std::filesystem::create_directories(dir);
    ...
    check(skiffllm::write_config_file(cfg.config_path, cfg, error), "config should be written");
    ...
    check(skiffllm::metrics_path_for(cfg) == (dir / "metrics.txt"),
    ...
    skiffllm::record_generation(cfg, 10, 50, 100.0, 500.0, 100.0);
    skiffllm::record_generation(cfg, 5, 20, 80.0, 320.0, 62.5);

    skiffllm::UsageStats stats;
    check(skiffllm::load_usage_stats(cfg, stats, error), "usage stats should load");
...
void test_openai_passthrough_flags() {
    skiffllm::Config cfg = skiffllm::default_config();
    std::vector<std::string> storage = {
        "skiffllm",     "--base-url", "http://127.0.0.1:8080",
        "--model",      "local",      "--stream",
...
void test_ui_and_backend_flags() {
    ...
    skiffllm::Config cfg2 = skiffllm::default_config();
    std::vector<std::string> storage2 = {"skiffllm", "--context-bar"};
    auto args2 = args_from(storage2);
    check(skiffllm::parse_args(static_cast<int>(args2.size()), args2.data(), cfg2, error),
...
void test_server_and_benchmark_flags() {
    skiffllm::Config cfg = skiffllm::default_config();
    std::vector<std::string> storage = {
        "skiffllm", "--serve",   "--host",      "0.0.0.0",     "--port",
...
void test_session_resolution() {
    skiffllm::Config cfg = skiffllm::default_config();
    std::vector<std::string> storage = {"skiffllm", "--session", "writing"};
    auto args = args_from(storage);
    std::string error;
    check(skiffllm::parse_args(static_cast<int>(args.size()), args.data(), cfg, error),
...
void test_cli_utils() {
    ...
    ok = false;
    check(skiffllm::cli::parse_int("42", ok) == 42, "parse_int should parse valid values");
    check(ok, "parse_int should report success");
    skiffllm::cli::parse_int("-1x", ok);
    check(!ok, "parse_int should reject invalid values");

    check(skiffllm::cli::compact_preview("a\nb\tc", 2) == "a ...",
          "compact_preview should replace whitespace");
    check(skiffllm::cli::json_escape("a\"b\\c") == "\"a\\\"b\\\\c\"",
          "json_escape should escape quotes and backslashes");

    std::string error;
    check(skiffllm::cli::make_attach_block({}, error).empty(), "empty attach list should be empty");
